...
#ifndef _GAUSSFILTER_COSIM_INCLUDED_
#define _GAUSSFILTER_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct GaussFilter_cosim : public ncsc_foreign_module
#else
SC_MODULE(GaussFilter_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module GaussFilter is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of GaussFilter in SystemC\n", 
					scName );
				return;
			} else {
				s << "GaussFilter0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module GaussFilter are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		int o_rgb_busy_is_clock = esc_link_clockgen( &o_rgb_busy, sc_time( 0, SC_NS ), module_path, sim_domain, "o_rgb_busy" );
		...
		esc_link_signals( &o_rgb_busy, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &o_rgb_vld, module_path, sim_domain, true );
		esc_link_signals( &o_rgb_data, module_path, sim_domain, true );
		...
		if ( i_clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussFilter", "i_clk", scName ) ) {
			esc_report_error( esc_error, "The port 'i_clk' on module 'GaussFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_rst_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussFilter", "i_rst", scName ) ) {
			esc_report_error( esc_error, "The port 'i_rst' on module 'GaussFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_rgb_vld_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussFilter", "vld", scName ) ) {
			esc_report_error( esc_error, "The port 'vld' on module 'GaussFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( o_rgb_busy_is_clock )
		{
			sc_signal_in_if< bool > *clock_inif = o_rgb_busy[0];
			sc_clock *clock_module = dynamic_cast<sc_clock*>(clock_inif);
			esc_hub_register_clock( clock_module, esc_alledge, 0, module_path, sim_domain, o_rgb_busy.basename() );
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussFilter", "busy", scName ) ) {
			esc_report_error( esc_error, "The port 'busy' on module 'GaussFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module GaussFilter must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "GaussFilter_nc_cosim"; } 
    GaussFilter_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	GaussFilter_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" GaussFilter") ) )
		: sc_module(in_name)
#endif
		  ,i_clk("i_clk")
		  	,i_rst("i_rst")
...
int GaussFilter_cosim::numLinked = 0;
GaussFilter_cosim::StringPair GaussFilter_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
