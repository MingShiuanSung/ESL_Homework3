...
void Testbench::fetch_result() {
  ...
  sc_dt::sc_uint<24> rgb; // double rgb
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = rgb.range(7,0);
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = rgb.range(15,8);
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = rgb.range(23,16);
    }
  }
